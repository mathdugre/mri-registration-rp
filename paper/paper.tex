...
\begin{abstract}
	Image registration is a critical step in MRI analysis pipelines, yet its
	high computational cost limits its use in large population cohorts and
	...
	accuracy. Its success in deep learning has driven broad adoption, but uptake remains
	limited in other scientific domains due to a limited understanding of its
	effects in complex applications. In this
	work, we study the effects of mixed precision in ANTs
	...
	sufficient in ANTs registration. Low-precision formats such as BF16 can even
	be used in early stages without loss of accuracy, provided that the final
	stage maintains sufficient precision (e.g., FP24). \HL{Furthermore, we
	demonstrate that our model accurately predicts precision requirements in
	practice, even when the theoretical assumptions for guaranteed convergence
	are not strictly met.} Our work characterizes when mixed precision is beneficial in ANTs
	registration, laying the groundwork for practical pipeline acceleration.
	Our theoretical model also provides a principled approach to dynamically
	selecting precision in ANTs registration.
\end{abstract}
...
Image registration maps images to a common space, enabling comparison across
images acquired at different times, from different subjects, or in different
modalities. This underpins longitudinal studies, group analyses, and multi-modal
imaging. Several toolkits implement MRI registration for neuroimaging, including
FSL~\cite{Jenkinson2012-tq}, SPM~\cite{Ashburner1997-fh}, and ANTs~\cite{Avants2020-xx}.

MRI registration incurs a significant computational cost, often accounting for a
majority of the runtime for MRI processing pipelines. As a result, optimizing
registration performance is crucial for improving the overall efficiency of MRI
processing, enabling studies with large population cohorts and clinical applications
requiring processing in a timely manner. Our previous study~\cite{Dugre2025-fs}
identified two primary performance bottlenecks in MRI processing pipelines: (1)
interpolation computation and (2) memory bandwidth
limitations.
...
formats with smaller bit \HL{widths allow for faster computation} and reduced
memory footprint. Therefore, \HL{mixed precision} has the potential to alleviate
both bottlenecks. However, it is unclear when and where it can be safely applied
in complex scientific pipelines such as medical image registration without
compromising result \HL{accuracy}.

We conduct a detailed
analysis of mixed precision formats in the context of ANTs registration
pipeline~\cite{Avants2020-xx}. This work contributes to the understanding and
application of mixed precision in MRI registration through the following:
\begin{itemize}
...
introduces background concepts and discusses related work on
...
but \HL{is also highly data-dependent}. Misalignments or topological distortions
...
\end{equation}
the optimization is guaranteed to progress toward the minimum \HL{if}
the gradient signal $\|\nabla \mathcal{L}(\theta)\|$ exceeds the rounding noise.
The theorem shows that if the \HL{optimization} does not converge to the minimum,
then it will reach a critical \HL{region} bounding the loss gradient
norm as follows:
\begin{equation}
...
inequality required to guarantee $\mathcal{L}(\theta_{t+1}) <
	\mathcal{L}(\theta_t)$. By taking the contrapositive, we establish a sufficient\HL{,}
\HL{but not necessary,} condition for convergence. \HL{If} the gradient signal is not
masked by precision-induced noise, \HL{the optimization will converge. Thus, it is
	sufficient that} the precision $p$ satisfies:
\begin{equation}
...
$\|\nabla \mathcal{L}\|$, and $\|\theta\|$ at each iteration. As in the \HL{study
	by Hayford et al.}~\cite{Hayford2024-kb}, $K$ was estimated using a first-order
finite difference:
\begin{equation}
	K \approx \frac{\|\nabla \mathcal{L}(\theta_{t+1}) - \nabla \mathcal{L}(\theta_t)\|}{\|\theta_{t+1} - \theta_t\|}
\end{equation}
By computing Equation~\ref{eq:pmin} dynamically, we determine the minimum
precision per iteration necessary to maintain the accuracy of the ANTs
...
propagated from preceding steps. \HL{Consequently, low-precision formats such as
	BF16 could potentially be used for the entire pipeline if only the final
	transformation from the SyN stage is required, rather than
	intermediate aligned volumes.}
