...
\begin{abstract}
	...
	or more images. Its high compute cost limits analysis of large population cohorts and
	...
	sufficient in ANTs registration. Low-precision formats such as BF16 can even be used in early stages without loss of accuracy, provided that the final stage maintains sufficient precision (e.g., FP24). 
	\HL{Furthermore, we demonstrate that our model accurately predicts precision requirements in practice, even when the theoretical assumptions for guaranteed convergence are not strictly met.}
	Our work advances the understanding of mixed precision in ANTs registration
	by characterizing when \HL{mixed precision} is beneficial, \HL{thus laying the
		groundwork} for practical acceleration of the pipeline. Furthermore, our
	theoretical model offers a simple approach to \HL{dynamically} optimizing the
	precision in ANTs registration.
\end{abstract}
...
resonance imaging (MRI) processing pipelines as it \HL{enables} the comparison of
images acquired at different times, from different subjects, or from different
modalities. \HL{This alignment enables} longitudinal studies, group analysis, or multi-modal imaging.
\HL{Consequently,} registration is an important step in toolkits such as the Advanced
...
requiring processing in timely manner. Our previous study~\cite{Dugre2025-fs}
identified two primary performance bottlenecks in MRI processing pipelines: (1)
interpolation computation in the registration process and (2) memory bandwidth
limitations.
...
formats with smaller bit \HL{widths allow for faster computation} and
reduced memory footprint. Therefore, \HL{mixed precision} has the potential to
alleviate both bottlenecks. However, while mixed precision has the potential for significant
