...
\begin{abstract}
	...
	stage maintains sufficient precision (e.g., FP24). Furthermore, we
...
formats with smaller bit widths allow for faster computation and reduced
memory footprint. Therefore, mixed precision has the potential to alleviate
...
but is also highly data-dependent. Misalignments or topological distortions
...
\end{equation}
the optimization is guaranteed to progress toward the minimum if
the gradient signal $\|\nabla \mathcal{L}(\theta)\|$ exceeds the rounding noise.
The theorem shows that if the optimization does not converge to the minimum,
then it will reach a critical region bounding the loss gradient
norm as follows:
\begin{equation}
...
but not necessary, condition for convergence. If the gradient signal is not
masked by precision-induced noise, the optimization will converge. Thus, it is
...
propagated from preceding steps. Consequently, low-precision formats such as
