...
\begin{abstract}
	...
	sufficient in ANTs registration. However, while our model can predict
	...
	by characterizing when mixed precision is beneficial, thus laying the
	groundwork for practical acceleration of the pipeline. Furthermore, our
	theoretical model offers a simple approach to dynamically optimizing the
...
resonance imaging (MRI) processing pipelines as it enables the comparison of
images acquired at different times, from different subjects, or from different
modalities. This alignment enables longitudinal studies, group analysis, or multi-modal imaging.
Consequently, registration is an important step in toolkits such as the Advanced
...
formats with smaller bit widths allow for faster computation and
reduced memory footprint. Therefore, mixed precision has the potential to
...
then it will reach a critical region \TG{``the model''
...
reproducibility across CPU microarchitectures. The code used for these
